# Step 2 of 6
# merge combined ESS data file with regional data,
...
# Ireland 10
# coarse 3 cat coding (because of ESS3)
	# 101 Dublin
	# 102 Border, Midlands and West
	# 103 Southern and Eastern, excl. Dublin
...
## income distance from cntryyear mean in percent
mean_inc <- aggregate(dat$natincome2005, list(dat$cntryyear), mean, na.rm=TRUE)
colnames(mean_inc) <- c("cntryyear", "mean_income")
dat <- merge(dat, mean_inc, by="cntryyear")

dat$incdist <- ((dat$natincome2005-dat$mean_income)/dat$mean_income)*100
...
dim(dat)
dat <- merge(x=dat, y=regio, by="region", all=TRUE)
dim(dat)

write.dta(dat, file="DataRobust13.dta")

system("rm working.dta")
