# Step 4 of 6
# merge imputed data files with regional
...
# Load data with 5 imputations
load("3__imputeMVN.Rdata")



# =========================================================
# =  function for recoding region and numeric country var =
# =========================================================

# creates a numeric region ID variable, harmonized over
# different ESS waves
...
# ===========================================
# =  function for recoding of covariates    =
# ===========================================


recode.micro <- function(dat){
...
# =============================
# =  Run recode functions     =
# =============================

imp1 <- region(imps$imp$imp1)
imp2 <- region(imps$imp$imp2)
imp3 <- region(imps$imp$imp3)
imp4 <- region(imps$imp$imp4)
imp5 <- region(imps$imp$imp5)

imp1 <- recode.micro(imp1)
imp2 <- recode.micro(imp2)
imp3 <- recode.micro(imp3)
imp4 <- recode.micro(imp4)
imp5 <- recode.micro(imp5)
...
# Load regional data
regio <- read.delim("RegionalData.csv", sep = ",", header=TRUE)
regio$cntry <- NULL; regio$nutsid <- NULL

# Merge data with regional data & gini
imp1 <- merge(x=imp1, y=regio, by="region", all.x=TRUE)
imp2 <- merge(x=imp2, y=regio, by="region", all.x=TRUE)
imp3 <- merge(x=imp3, y=regio, by="region", all.x=TRUE)
imp4 <- merge(x=imp4, y=regio, by="region", all.x=TRUE)
imp5 <- merge(x=imp5, y=regio, by="region", all.x=TRUE)

# Save imputed datasets 1 to 5
write.dta(imp1, file="data_imp1.dta")
write.dta(imp2, file="data_imp2.dta")
write.dta(imp3, file="data_imp3.dta")
write.dta(imp4, file="data_imp4.dta")
write.dta(imp5, file="data_imp5.dta")
